...
class ChannelProject2D(nn.Module):
    def __init__(self, in_dim, out_dim, norm_layer=nn.LayerNorm, channel_first=False):
        super().__init__()
        self.channel_first = channel_first

        Linear = Linear2d if channel_first else nn.Linear
        Norm = LayerNorm2d if channel_first else nn.LayerNorm
...
class WeightedTokenMerge2D(nn.Module):
    def __init__(self, channel_first=False, tau=1.0):
        super().__init__()
        self.channel_first = channel_first
        self.tau = tau

    def forward(self, x: torch.Tensor, score_map: torch.Tensor = None):
        """
        x:
            channel_first=True  -> [B, C, H, W]
            channel_first=False -> [B, H, W, C]

        score_map:
            optional
            channel_first=True  -> [B, 1, H, W]
            channel_first=False -> [B, H, W, 1] or [B, H, W]

        return:
            merged tensor with spatial size H, W/2
        """
        # [B, C, H, W] -> [B, C, H, W/2]
        if self.channel_first:
            ...
            if pad_w:
                ...
                if score_map is not None:
                    score_map = F.pad(score_map, (0, pad_w, 0, 0))

            B, C, H, W = x.shape
            W2 = W // 2

            x_win = x.view(B, C, H, W2, 2)
            x_win = x_win.permute(0, 2, 3, 4, 1).reshape(B, H, W2, 2, C)

            if score_map is None:
                w = x_win.new_full((B, H, W2, 2), 0.5)
            else:
                s = score_map.view(B, 1, H, W2, 2)
                s = s.permute(0, 2, 3, 4, 1).reshape(B, H, W2, 2)
                w = torch.softmax(s / self.tau, dim=-1)

            y = (x_win * w.unsqueeze(-1)).sum(dim=3)
            ...
            if pad_h or pad_w:
                ...
                if score_map is not None:
                    ...
                    score_map = F.pad(score_map, (0, 0, 0, pad_w, 0, pad_h))

            B, H, W, C = x.shape
            H2, W2 = H // 2, W // 2

            # [B,H,W,C] -> [B,H2,W2,4,C]
            x_win = x.view(B, H2, 2, W2, 2, C)
            x_win = x_win.permute(0, 1, 3, 2, 4, 5).reshape(B, H2, W2, 4, C)

            if score_map is None:
                w = x_win.new_full((B, H2, W2, 4), 0.25)
            else:
                if score_map.dim() == 4 and score_map.shape[-1] == 1:
                    score_map = score_map.squeeze(-1)
                # [B,H,W] -> [B,H2,W2,4]
                s = score_map.view(B, H2, 2, W2, 2)
                s = s.permute(0, 1, 3, 2, 4).reshape(B, H2, W2, 4)
                w = torch.softmax(s / self.tau, dim=-1)

            y = (x_win * w.unsqueeze(-1)).sum(dim=3)   # [B,H2,W2,C]
            return y.contiguous()

def compute_proxy_dis_map(x: torch.Tensor, channel_first: bool = True, eps: float = 1e-6):
    """
    x:
        channel_first=True  -> [B, C, H, W]
        channel_first=False -> [B, H, W, C]
    ...
    return:
        dis_map
        channel_first=True  -> [B, 1, H, W]
        channel_first=False -> [B, H, W, 1]
    """
    if channel_first:
        # [B, C, H, W]
        dx_l = torch.zeros_like(x)
        dx_r = torch.zeros_like(x)
        dy_u = torch.zeros_like(x)
        dy_d = torch.zeros_like(x)
        ...
        # 방향별 변화량 평균
        d1 = dx_l.mean(dim=1, keepdim=True)
        d2 = dx_r.mean(dim=1, keepdim=True)
        d3 = dy_u.mean(dim=1, keepdim=True)
        d4 = dy_d.mean(dim=1, keepdim=True)

        ds = torch.cat([d1, d2, d3, d4], dim=1)   # [B,4,H,W]
        mu = ds.mean(dim=1, keepdim=True)
        dis = ((ds - mu) ** 2).mean(dim=1, keepdim=True).sqrt()  # std-like inconsistency

        # sample-wise normalize to [0,1]
        B = dis.shape[0]
        dis_flat = dis.view(B, -1)
        dis_min = dis_flat.min(dim=1)[0].view(B, 1, 1, 1)
        dis_max = dis_flat.max(dim=1)[0].view(B, 1, 1, 1)
        dis = (dis - dis_min) / (dis_max - dis_min + eps)
        ...
        ds = torch.cat([d1, d2, d3, d4], dim=-1)  # [B,H,W,4]
        mu = ds.mean(dim=-1, keepdim=True)
        dis = ((ds - mu) ** 2).mean(dim=-1, keepdim=True).sqrt()
...
class StageWithMerge(nn.Module):
    def __init__(
        self,
        blocks_pre,
        merge,
        ...
        super().__init__()
        self.blocks_pre = nn.Sequential(*blocks_pre)
        self.merge = merge
        self.blocks_post = nn.Sequential(*blocks_post)
    ...
    def forward(self, x):
        ...
        # 마지막 pre-block의 SS2D에서 저장한 진짜 delta-based DIS 사용
        last_blk = self.blocks_pre[-1]
        dis_map = getattr(last_blk.op, "last_dis_map", None)

        if dis_map is None:
            dis_map = compute_proxy_dis_map(x, channel_first=self.channel_first)
        #else:
            #print("DIS source: delta")

        #print("dis_map:", dis_map.shape, dis_map.min().item(), dis_map.max().item())

        if self.channel_first:
            feat_norm = x.norm(dim=1, keepdim=True)
            B = feat_norm.shape[0]
            feat_flat = feat_norm.view(B, -1)
            feat_min = feat_flat.min(dim=1)[0].view(B, 1, 1, 1)
            feat_max = feat_flat.max(dim=1)[0].view(B, 1, 1, 1)
            feat_norm = (feat_norm - feat_min) / (feat_max - feat_min + 1e-6)
        else:
            feat_norm = x.norm(dim=-1, keepdim=True)
            B = feat_norm.shape[0]
            feat_flat = feat_norm.view(B, -1)
        ...
        alpha = self.alpha
        score_map = self.alpha * (1.0 - dis_map) + (1.0 - self.alpha) * feat_norm # 중요도점수 바꿔도 ok
        x = self.merge(x, score_map=score_map)

        o = self.blocks_post(x)   # stage의 출력 feature
        y = self.downsample(o)    # 다음 stage로 넘길 feature
...
class StageWithMultiMerge(nn.Module):
    def __init__(
        self,
        blocks,
        merge_points,
        ...
        self.merge_points = set(merge_points)
        self.merge = merge
        self.downsample = downsample
        self.channel_first = channel_first
        self.alpha = alpha

    def _make_score_map(self, x, last_blk):
        dis_map = getattr(last_blk.op, "last_dis_map", None)
        ...
        score_map = self.alpha * (1.0 - dis_map) + (1.0 - self.alpha) * feat_norm
        return score_map
    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            # idx는 0부터 시작하므로, block 7 뒤에서 merge하려면 idx+1 == 7
            if (idx + 1) in self.merge_points:
                score_map = self._make_score_map(x, last_blk)
                x = self.merge(x, score_map=score_map)
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        # delta-based DIS cache
        self.last_dt_dir_map = None   # [B, 4, H, W]
        self.last_dis_map = None      # [B, 1, H, W]
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            if no_einsum:
                ...
                if hasattr(self, "dt_projs_weight"):
                    dts = torch.einsum("b k r l, k d r -> b k d l", dts, self.dt_projs_weight)  # [B,K,D,L]

            # delta-based DIS 저장
            dts_for_dis = dts

            # [B, K, D, L] -> [B, K, L]
            dt_token = dts_for_dis.abs().mean(dim=2)

            # 방향 간 표준편차 = DIS
            # [B, K, L] -> [B, 1, L]
            dis = dt_token.std(dim=1, keepdim=True)

            # sample-wise min-max normalize
            dis_min = dis.min(dim=-1, keepdim=True)[0]
            dis_max = dis.max(dim=-1, keepdim=True)[0]
            dis = (dis - dis_min) / (dis_max - dis_min + 1e-6)

            self.last_dt_dir_map = dt_token.view(B, K, H, W).detach()
            self.last_dis_map = dis.view(B, 1, H, W).detach()

            xs = xs.view(B, -1, L)
            dts = dts_for_dis.contiguous().view(B, -1, L)
...
class VSSM(nn.Module):
    def __init__(
        ...
        merge_stage=2,
        merge_after_block=10,
        merge_after_blocks=None,
        merge_tau=1.0,
        merge_alpha=0.6,
        merge_downsample=False,
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            drop_path_i = dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])]
            depth_i = len(drop_path_i)

            # -----------------------------
            # stage3만 custom stage로 생성
            # i_layer == 2  <=> stage3
            # -----------------------------
            if use_token_merge and i_layer == merge_stage:
                if merge_after_blocks is None:
                    merge_points = [int(x) for x in merge_after_block]
                else:
                    merge_points = list(merge_after_blocks)

                # 범위 보정
                merge_points = [int(p) for p in merge_points]
                merge_points = sorted(set([max(1, min(p, depth_i - 1)) for p in merge_points]))
                ...
                merge = WeightedTokenMerge2D(
                    channel_first=self.channel_first,
                    tau=merge_tau,
                )

                stage = StageWithMultiMerge(
                    blocks=blocks,
                    merge_points=merge_points,
                    merge=merge,
                    downsample=(
                        downsample if merge_downsample else
                        ChannelProject2D(
                            self.dims[i_layer],
                            self.dims[i_layer + 1],
                            norm_layer=norm_layer,
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        def layer_forward(l, x):
            if isinstance(l, StageWithMerge):
                o, y = l(x)
                return o, y
            else:
                o = l.blocks(x)
        ...
        for i, layer in enumerate(self.layers):
            o, x = layer_forward(layer, x)  # o: stage output before downsample, x: next input
